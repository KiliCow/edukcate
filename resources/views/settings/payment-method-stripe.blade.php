<edukcate-payment-method-stripe :user="user" :team="team" :billable-type="billableType" inline-template>
    <div>
        <!-- Current Discount -->
        ...
        @if (Edukcate::collectsEuropeanVat())
            @include('edukcate::settings.payment-method.update-vat-id')
        @endif

        <!-- Update Card -->
        @include('edukcate::settings.payment-method.update-payment-method-stripe')

        <div>
            <div v-if="billable.stripe_id">
                <!-- Redeem Coupon -->
                @include('edukcate::settings.payment-method.redeem-coupon')
            </div>
        </div>
    </div>
</edukcate-payment-method-stripe>
