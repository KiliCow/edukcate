...
@section('content')
    <edukcate-settings :user="user" :teams="teams" inline-template>
        <div class="edukcate-screen container">
            <div class="row">
                <!-- Tabs -->
                <div class="col-md-3 edukcate-settings-tabs">
                    <aside>
              ...
              18c2.7737 0 5.2177-1.4116 6.6528-3.5556C14.6268 13.517 12.3738 13 10 13s-4.627.517-6.6528 1.4444z "></path>
                            ...
                            @if (Edukcate::usesTeams())
                                <li class="nav-item ">
                                    <a class="nav-link" href="#{{Edukcate::teamsPrefix()}}" aria-controls="teams" role="tab" data-toggle="tab">
              ...
              1-2 2-2h4V2c0-1 1-2 2-2s2 1 2 2v2h4c1 0 2 1 2 2v4h-2c-1 0-2 1-2 2s1 2 2 2h2z "/>
                    ...
                    <!-- Billing Tabs -->
                    @if (Edukcate::canBillCustomers())
                        <aside>
                            <h3 class="nav-heading ">
                                {{__('Billing')}}
                            </h3>
                            <ul class="nav flex-column mb-4 ">
                                @if (Edukcate::hasPaidPlans())
                                    <li class="nav-item ">
                ...
                <div class="col-md-9">
                    <div class="tab-content">
                        ...
                        <div role="tabcard" class="tab-pane active" id="profile">
                            @include('edukcate::settings.profile')
                        </div>

                        <!-- Teams -->
                        @if (Edukcate::usesTeams())
                            <div role="tabcard" class="tab-pane" id="{{Edukcate::teamsPrefix()}}">
                                @include('edukcate::settings.teams')
                            </div>
                        @endif

                    <!-- Security -->
                        <div role="tabcard" class="tab-pane" id="security">
                            @include('edukcate::settings.security')
                        </div>

                        <!-- API -->
                        @if (Edukcate::usesApi())
                            <div role="tabcard" class="tab-pane" id="api">
                                @include('edukcate::settings.api')
                            </div>
                        @endif
                    ...
                    <!-- Billing Tab Panes -->
                        @if (Edukcate::canBillCustomers())
                            ...
                            <!-- Subscription -->
                                <div role="tabcard" class="tab-pane" id="subscription">
                                    <div v-if="user">
                                        @include('edukcate::settings.subscription')
                                    </div>
                                </div>
                            @endif
                        ...
                        <!-- Payment Method -->
                            <div role="tabcard" class="tab-pane" id="payment-method">
                                <div v-if="user">
                                    @include('edukcate::settings.payment-method')
                                </div>
                            </div>

                            <!-- Invoices -->
                            <div role="tabcard" class="tab-pane" id="invoices">
                                @include('edukcate::settings.invoices')
                            </div>
                        @endif
                    </div>
                </div>
