...
namespace KiliCow\Edukcate\Http\Middleware;

use Closure;
use KiliCow\Edukcate\Edukcate;
use Illuminate\Http\Response;
use KiliCow\Edukcate\Contracts\Repositories\TokenRepository;
...
class CreateFreshApiToken
{
    ...
    {
        return Edukcate::usesApi() && $this->requestShouldReceiveFreshToken($request) &&
    ...
    /**
     * Determine if the given response already contains a Edukcate API token.
     *
    ...
    protected function alreadyContainsToken($response)
    {
        foreach ($response->headers->getCookies() as $cookie) {
            if ($cookie->getName() === 'edukcate_token') {
                return true;
            }
        }
