...
<edukcate-navbar
    ...
    <nav class="navbar navbar-light navbar-expand-md navbar-edukcate">
        <div class="container" v-if="user">
            <!-- Branding Image -->
            @include('edukcate::nav.brand')
            ...
            <div id="navbarSupportedContent" class="collapse navbar-collapse">
                <ul class="navbar-nav mr-auto">
                    @includeIf('edukcate::nav.user-left')
                </ul>
                    ...
                    <li class="nav-item dropdown">
                        <a href="#" class="d-block d-md-flex text-center nav-link dropdown-toggle" id="dropdownMenuButton" data-toggle="dropdown"
                           aria-haspopup="true" aria-expanded="false">
                            <img :src="user.photo_url" class="dropdown-toggle-image edukcate-nav-profile-photo">
                            <span class="d-none d-md-block">@{{ user.name }}</span>
                        </a>
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                            <!-- Impersonation -->
                            @if (session('edukcate:impersonator'))
                                <h6 class="dropdown-header">{{__('Impersonation')}}</h6>

                                <!-- Stop Impersonating -->
                                <a class="dropdown-item" href="/edukcate/kiosk/users/stop-impersonating">
                            ...
                            <!-- Developer -->
                            @if (Edukcate::developer(Auth::user()->email))
                                @include('edukcate::nav.developer')
                            @endif

                            <!-- Subscription Reminders -->
                            @include('edukcate::nav.subscriptions')
                            ...
                            @if (Edukcate::usesTeams() && (Edukcate::createsAdditionalTeams() || Edukcate::showsTeamSwitcher()))
                                <!-- Team Settings -->
                                @include('edukcate::nav.teams')
                            @endif

                            @if (Edukcate::hasSupportAddress())
                                <!-- Support -->
                                @include('edukcate::nav.support')
                            @endif
