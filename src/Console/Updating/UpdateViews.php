...
class UpdateViews
{
    ...
    {
        $views = collect(
            (new Filesystem)->allFiles(EDUKCATE_PATH.'/resources/views')
        );
    ...
    /**
     * Publish the Edukcate views that exist in the download but are not published.
     *
     * @return void
     */
    ...
    {
        return resource_path(implode(DIRECTORY_SEPARATOR, ['views', 'vendor', 'edukcate', $this->relativeViewPath($view)]));
    }
    ...
    {
        $edukcatePath = implode(DIRECTORY_SEPARATOR, [EDUKCATE_PATH, 'resources', 'views']);

        $downloadPath = implode(DIRECTORY_SEPARATOR, [$this->downloadPath, 'resources', 'views']);

        return str_replace(
            [$edukcatePath.DIRECTORY_SEPARATOR, $downloadPath.DIRECTORY_SEPARATOR],
            '',
            $view->getRealPath()
        );
    }
